...
class ResNetPointCloudBase(torch.nn.Module):
    ...
    def add_subsampled_pos(self, data):
        # Sample kernel points and positions.
        kernel_pos, kernel_pos_idx = self.conv1.sample_kernel_points(data.x, data.pos)

        # Reshape kernel pos from [num_graphs, num_nodes, num_edges, 3] to [num_graphs * num_nodes * num_edges, 3].
        data.kernel_pos = kernel_pos.view(
            data.x.shape[0] * data.x.shape[1] * kernel_pos.shape[2], 3, 1, 1, 1
        )

        # Add kernel position indices to databatch object.
        data.kernel_pos_idx = kernel_pos_idx
        return data
...
class ResNet_pointcloud(ResNetPointCloudBase):
    def forward(self, data):
        # Add sampled kernel points to databatch object.
        self.add_subsampled_pos(data)
